            ...
            for (TUINT32 udwIdy = 0; udwIdy < jMemberVersion.size(); ++udwIdy)
            {
                ...
                for (TUINT32 udwIdz = 0; udwIdz < jMemberPlatform.size(); ++udwIdz)
                {
                    ...
                    for (TUINT32 udwIdm = 0; udwIdm < jMemberFile.size(); ++udwIdm)
                    {
                        ...
                        pobjContent->m_udwFileTs = jDataListJson["versionconfig"][jMemberPlatform[udwIdy]][jMemberFile[udwIdz]]["timestamp"].asUInt();
...
TINT32 CStaticFileMgr::LoadStaticFile( CStaticFileContent *pobjStaticFile )
{
    ...
    if(IsFIleTypeNeedLoadData(pobjStaticFile->m_strStaticDataType))
    {
        std::ifstream is;
        Json::Reader reader;
...
TINT32 CStaticFileMgr::InitStaticFileProperty()
{
    ...
    AddStaticFileProperty(EN_STATIC_TYPE_FUNCTION_SWITCH, false, false);
    ...
    AddStaticFileProperty(EN_STATIC_TYPE_META_RAW, true, true);
...
TINT32 CStaticFileMgr::AddStaticFileProperty( string strFileType, TBOOL bDiffPlatForm, TBOOL bDiffVersion, TBOOL bNeedLoadData )
{
...
TINT32 CStaticFileMgr::UpdtAndGetMaintainStatus( Json::Value &jMaintainJson, string strPlatForm, string strVersion, string strSid, TUINT32 udwCurTime )
{
    ...
    // global——强制更新
    if (atof(strVersion.c_str()) + 0.001f < jMaintainJson["global"]["min_ver"].asFloat())
    {
        jMaintainJson["global"]["status"] = EN_MAINTAIN_TYPE_VERSION_FORCE_UPDATE;
        dwRetStatus = EN_MAINTAIN_TYPE_VERSION_FORCE_UPDATE;
    }
    else
    {
    ...
    else if (udwMaintainBeg <= udwCurTime && udwCurTime <= udwMaintainEnd)
    {
        ...
        dwRetStatus = EN_MAINTAIN_TYPE_GLOBAL_MAINTAIN;
    }
    else
    {
    ...
    {
        if(jMaintainJson["svrList"][strSid]["status"].asUInt() == 2)
        {
            dwRetStatus = EN_MAINTAIN_TYPE_SVR_MAINTAIN;
        }
    }
